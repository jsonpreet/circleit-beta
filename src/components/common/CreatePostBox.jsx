...
export default function CreatePostBox({ circle }) {
  ...
  const submitPost = async () => {
    ...
    try {
      ...
      let body = postBody.trim().length > 0 ? postBody + `\n\n Posted on @CircleIt in @${circle.Username}` : null;
      const request = {
  ...
  return (
    <>
      <div onClick={() => setExpand(true)} className='mt-4 flex-col flex secondaryBg primaryBorder border rounded-md'>
        <div className='flex items-start space-x-0 md:space-x-4 my-4 px-4 md:px-6'>
          ...
          <div className='flex w-full flex-col'>
            <div className="flex items-center">
              <input
                className={`focus:ring-0 focus:outline-none outline-none darkenBg darkenHoverBg border dark:border-[#2D2D33] hover:dark:border-[#43434d] border-gray-200 hover:border-gray-200 resize-none w-full heading px-4 py-2 ${isExpanded ? 'rounded-md' : 'rounded-full'}`}
              ...
              <>
                  ...
                  {isVideoReady && postVideo !== '' ?
                    <div className="relative mt-4">
                      <div className='mt-2 relative pt-[56.25%] w-full rounded-xl max-h-[700px] overflow-hidden'>
                        <iframe src={postVideo} className='w-full absolute left-0 right-0 top-0 bottom-0 h-full' allow="accelerometer; gyroscope; autoplay; encrypted-media; picture-in-picture;" allowFullScreen></iframe>
                    </div>
                      <div className="absolute top-4 right-4 ">
                        <button onClick={() => {
                            setPostVideo('')
                  ...
                  {!isVideoReady && postVideo !== '' && 
                    <>
                      <div className="flex mt-4 space-x-2 items-center">
                        <Loader className='w-5 h-5' /> <span>Processing Video</span>
                      </div>  
                    </>
                  }
                  {/* !isVideoReady && postVideo === '' &&  */}
                  {!isVideoReady && postVideo === '' &&  uploadProgress > 0 && <ProgressBar bgcolor="#ff00ff" progress={uploadProgress} height={24} />}
                </div>
                <div className="flex items-center mt-2 justify-between w-full">
                  ...
                  <div className="justify-end">
                    <button onClick={submitPost} className='font-medium text-white transition delay-75 px-6 py-1.5 rounded-full buttonBG ml-6'>
