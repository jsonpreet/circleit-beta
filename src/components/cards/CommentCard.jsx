...
import { dateFormat, LinkifyOptions, timeStampToTimeAgo } from '../../utils/Functions';
...
function CommentCard({post, comment, isSubComment, parent = null}) {
    ...
    useEffect(() => {
        ...
        if (comment.Body && comment.ProfileEntryResponse) {
            let body1 = comment.Body.replace(
                `Posted on @CircleIt in @${circle}`,
                ""
            );
            let body2 = body1.replace(`Posted on @${circle}`, "");
            let body3 = body2.replace(`Posted on @CircleIt in @${comment.ProfileEntryResponse.Username}`, "");
            let body = body3.replace(`in @undefined`, "");
            setCommentBody(body);
        }
        if (comment.ProfileEntryResponse.ExtraData !== null && comment.ProfileEntryResponse.ExtraData.CircleIt !== undefined) {
            const payload = JSON.parse(comment.ProfileEntryResponse.ExtraData.CircleIt);
            const isCircle = payload !== null && payload.isCircle === "true" ? true : false;
                ...
                <div className='flex space-y-3 md:divide-y-0 divide-y theme-divider md:space-y-0 space-x-0 md:space-x-3'>
                    {isBrowser ?
                        <div className='flex thread relative'>
                            <Link
                                to={`/${isCircle ? `circle` : `u`}/${comment.ProfileEntryResponse.Username
                                    }`}
                                ...
                                <img
                                    ...
                                    alt={comment.ProfileEntryResponse.ExtraData?.DisplayName ? comment.ProfileEntryResponse.ExtraData?.DisplayName : comment.ProfileEntryResponse.Username}
                                />
                            </Link>
                        </div> : null}
                    <div className='md:flex-1'>
                        <div className='flex items-center space-x-2'>
                            ...
                            <span className='text-sm font-semibold'>
                                 <Tippy  followCursor={true} placement="bottom" interactive={true} maxWidth={300} interactiveDebounce={100} delay={500} render={attrs => <SubProfileCard isCircle={false} profile={comment.ProfileEntryResponse} {...attrs}/>}>
                                    <Link
                        ...
                        {parent &&
                            <div className='text-sm'>
                                Replying to <Link to={`/${isCircle ? `circle` : `u`}/${parent.ProfileEntryResponse.Username}`} className='font-semibold brandGradientText text-sm'>{parent.ProfileEntryResponse.Username}</Link>
                            </div>
                        }
                        ...
                        {comment.ImageURLs?.length > 0 && (
                            <PostImages images={comment.ImageURLs} circle={comment.ProfileEntryResponse} />
                        )}
                        ...
                        {hasMore && comments.length > 0 && (                           
                            ...
                            <div data-id={isSubComment ? lastID : comment.PostHashHex} className='flex items-center justify-start mt-2'>
                                <button 
                                    className='text-sm brandGradientText font-medium'
                                    onClick={(e) => loadMore(e, isSubComment ? lastID : comment.PostHashHex)}
                                >
                                    {!loading ? `View more comments` : `Fetching more comments...`}
                                </button>
                            </div>
                        )}
