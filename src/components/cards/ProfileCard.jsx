...
function ProfileCard({circle}) {
    ...
    const cover = circle.ExtraData && circle.ExtraData.FeaturedImageURL !== '' ? circle.ExtraData.FeaturedImageURL : 'https://wallpaperaccess.com/full/1760835.jpg';
            ...
            <div className='rounded-md secondaryBg secondaryBorder secondaryTextColor overflow-hidden border relative z-0'>
                <div className='flex flex-col'>
                    <div className="h-32 md:h-48" 
                        style={{
                        position: "relative",
                        backgroundImage: `url(${cover !== '' ? cover : 'https://wallpaperaccess.com/full/1760835.jpg'})`,
                        backgroundSize: "cover",
                        backgroundPosition: "center",
                        }}>
                    </div>
                    {isLoggedIn && circle.PublicKeyBase58Check === user.profile.PublicKeyBase58Check && (
                        <div className="relative w-full flex z-20 items-end justify-end pt-4 pr-4">
                            <Tippy content='Account Settings'>
                                <button onClick={() => navigate(`${settingsURL}/${user.profile.Username}/settings`)} className='p-2 rounded-full bg-primaryBg hover:bg-primaryBgHover'>
                    ...
                    <div className='flex flex-col items-center justify-center -mt-20 relative z-10'>
                        <img
                            ...
                            className='rounded-full subHeader border-4 border-white dark:border-[#212126] w-24 h-24 md:w-32 md:h-32' alt={circle.Username}
                        />
                        <div className="flex items-center flex-col mt-1">
                            <h2 className='font-semibold text-2xl'>{circle.ExtraData?.DisplayName ? circle.ExtraData?.DisplayName : circle.Username}</h2>
                            <p className="text-gray-500 dark:text-gray-400 text-sm">{`${isCircle ? `circle` : `u`}/${circle.Username}`}</p>
                        </div>
                    </div>
                </div>
                <div className="p-6 pt-2">
                    <div className='w-full pt-2'>
                        <Linkify options={LinkifyOptions}>
                            {!readMore ? circle.Description : `${circle.Description.substring(0, 200)} `}
                        </Linkify>
                        {readMore && (
                            <span
                ...
                <div className="px-6 py-3 flex justify-between">
                    <div className="flex flex-col items-center space-y-1">
                        ...
                        <span className="text-sm">{isCircle ? `Members` : `Followers`}</span>
                    </div>
                    <div className="flex flex-col items-center space-y-1">
